...
* **Test schema is not built from Alembic:** `src/tests/conftest.py` creates tables from a hand-written `SCHEMA_SQL` and injects its own pool into `db.connection._pool` (bypassing `create_pool()`'s env-secret/`DB_SSL` path). Any migration that adds/changes a table or column must also update `SCHEMA_SQL` and the `clean_db` `TRUNCATE` list, or tests will run against a stale schema.
...
* **Rate limiting** (`services/rate_limit_service.py`, `repositories/rate_limit_repository.py`, Redis via `db/redis_connection.py`): fixed-window counters guard `/register`, `/login`, `/mfa/verify`, `/forgot-password`, `/resend-verification`, and `/oauth/google/login`+`/callback`. Each is limited both by client IP (`IP_LIMITS`, catches broad/scripted abuse) and, for the account-targeted actions, by the target account (`ACCOUNT_LIMITS`, keyed by email, or by user id for `mfa_verify`; catches targeted brute-force/spam on one account) — both checks run independently via `enforce_rate_limit()`, called from `api/v1/auth.py`. Exceeding either raises 429. `increment_rate_limit()` does `INCR` then an unconditional `EXPIRE ... NX` on the Redis key `ratelimit:{ip|account}:{action}:{key}` (`REDIS_URL` is required in production — `validate_redis_config()` in `main.py`'s lifespan) — TTL is set with `NX` on every call (not just when the count is 1) so a process dying between the two calls can't leave a key with no TTL, which would otherwise lock that IP/account out permanently. The Postgres `rate_limits` table from the prior implementation was dropped in a separate follow-up migration (`c73e4cb39f0b`), not in the deploy that switched to Redis, so the previous still-running task revision never lost its table mid-rollout.
* **Audit log** (`services/audit_service.py`, `repositories/audit_repository.py`, `audit_events` table, `GET /api/v1/admin/audit-events` behind `require_role("admin")`): append-only record of `login_success`/`login_failure` (with `method` or `reason` in `metadata`), `logout`, `refresh_token_reuse_detected`, `password_reset_completed`, `mfa_enabled`/`mfa_disabled`, `role_changed` (`actor_id`, `old_role`, `new_role`), and `oauth_account_linked`. Services call `record_event(event_type, user_id, **metadata)` at the point the event happens; client IP and user-agent come from `ContextVar`s set by the `capture_request_context` HTTP middleware (`core/request_context.py`, registered in `main.py`), so service signatures don't carry request info. Never record passwords, tokens, TOTP/recovery codes, or the email submitted on a failed login (unknown-account failures are stored with `user_id = NULL`). `event_type` is enforced both by the `AuditEventType` `Literal` in `schemas/admin.py` and a DB CHECK constraint, so a new event type needs a migration plus `SCHEMA_SQL` updates. `user_id` is `ON DELETE SET NULL` so events outlive their user; there's no cleanup job. Service unit tests mock the insert via the autouse `audit_events` fixture in `src/tests/test_services/conftest.py` (assert on it to check which events a service records).
...
* **Config/secrets:** `core/config.py` loads `JWT_SECRET_KEY`/`ALGORITHM` via `python-dotenv`. `core/secrets.py`'s `get_db_secret()` reads `DB_USERNAME`/`DB_PASSWORD`/`DB_HOST`/`DB_PORT`/`DB_NAME` (plus optional `DB_SSL`, default `require`; `.env.example` sets `disable` for the non-TLS local Postgres) directly from the environment (in ECS these are injected by Secrets Manager into the container env — the app does not call boto3 itself).
...
* **CD pipeline** (`.github/workflows/ci.yml`'s `deploy` job, `needs: test`, only on push to `main`): authenticates to AWS via GitHub OIDC role assumption (`aws-actions/configure-aws-credentials`, `role-to-assume: vars.AWS_DEPLOY_ROLE_ARN` — no static AWS keys stored in GitHub), builds and pushes the image tagged with the commit SHA (not `:latest`, since ECS won't reliably redeploy on a same-tag push), fetches the live ECS task definition and swaps in the new image, then runs `alembic upgrade head` as a **one-off ECS Fargate task** on the newly-registered revision (same VPC subnets/security group as the live service, so it reaches RDS the same way the app already does — no need to open the database to GitHub's runner IPs) before deploying that revision to the service with `wait-for-service-stability: true`. Requires a one-time IAM setup (GitHub OIDC provider + a role scoped to this repo's `main` branch, permissions covering ECR push, `ecs:RegisterTaskDefinition`/`RunTask`/`UpdateService`/`DescribeTasks`/`DescribeServices`, and `iam:PassRole` limited to `ecsTaskExecutionRole`) and a `vars.AWS_DEPLOY_ROLE_ARN` GitHub Actions variable — see SETUP.md. Because migrations run automatically before every deploy, a migration that isn't backward-compatible with the *previous* deploy's still-running code (e.g. dropping a column/table the old revision still reads) must ship as its own separate deploy, never bundled with the code change that stops depending on it.
