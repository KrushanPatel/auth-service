...
* **Email delivery** (`core/email.py`): the reset link is sent over SMTP via `schedule_password_reset_email()`, which fires `send_password_reset_email()` as a background `asyncio.Task` (held in a module-level set until done, so it isn't GC'd mid-send) rather than making `/forgot-password` wait on SMTP; delivery failures are swallowed (logged, never raised) since the endpoint's response must not reveal whether sending succeeded. If `SMTP_HOST` isn't set, it falls back to logging the raw reset link instead of emailing it. `validate_email_config()` runs at startup in `main.py`'s lifespan and raises if `ENV=production` with no `SMTP_HOST`, so a misconfigured prod deploy fails fast instead of silently degrading to log-only delivery.
* **Rate limiting** (`services/rate_limit_service.py`, `repositories/rate_limit_repository.py`, `rate_limits` table): fixed-window counters (not Redis — this app has no cache layer, and the check piggybacks on a request the app is already making) guard `/register`, `/login`, and `/forgot-password`. Each is limited both by client IP (`IP_LIMITS`, catches broad/scripted abuse) and, for `/login` and `/forgot-password`, by the target account email (`ACCOUNT_LIMITS`, catches targeted brute-force/spam on one account) — both checks run independently via `enforce_rate_limit()`, called from `api/v1/auth.py`. Exceeding either raises 429. `increment_rate_limit()` does the reset-or-increment atomically in one `INSERT ... ON CONFLICT DO UPDATE`.
* **Background cleanup:** `cleanup_task()` in `refresh_token_service.py` runs as an `asyncio` task started in `main.py`'s lifespan, deleting expired refresh tokens, expired password reset tokens, and stale rate-limit rows once per hour.
