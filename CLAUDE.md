...
* **Auth tokens** (`core/jwt.py`, PyJWT, HS256): access tokens expire in 15 min; refresh tokens in 7 days and carry a `jti`; password reset tokens expire in 30 min and also carry a `jti`. Refresh and password reset tokens are hashed with Argon2id (`core/security.py`, same hasher used for user passwords) before being stored via `repositories/refresh_token_repository.py` / `repositories/password_reset_repository.py`.
...
* **Password reset** (`services/password_reset_service.py`): `/api/v1/auth/forgot-password` always returns the same generic response regardless of whether the email is registered (no user enumeration); it logs the raw reset token server-side rather than emailing it, since no email delivery is wired up yet. `/api/v1/auth/reset-password` validates the single-use token, updates the password, and revokes all of that user's refresh tokens.
